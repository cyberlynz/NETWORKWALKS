...
The lab focused on installing Claude Desktop, downloading and preparing the HexStrike MCP server, starting the HexStrike server, and configuring Claude Desktop so that it could communicate with the local HexStrike MCP server. The objective was to establish a working AI-assisted cybersecurity lab environment. fileciteturn46file0L8-L19
...
The NetworkWalks guide identifies the lab environment as a **HexStrike-AI MCP server with Claude Desktop on a Kali Linux VM**. fileciteturn46file0L13-L19
...
These are the installation commands provided in the NetworkWalks guide. fileciteturn46file0L17-L29
...
The lab specifically instructs using the GitHub project and these commands to obtain the HexStrike-AI server files. fileciteturn46file0L35-L40
...
The virtual-environment commands are included in the NetworkWalks lab instructions. fileciteturn46file0L41-L43
...
The NetworkWalks guide specifies installing the dependencies from the project's `requirements.txt` file. fileciteturn46file0L44-L45
...
This started the HexStrike server locally. The lab later identifies the local server endpoint as **`http://localhost:8888`**. fileciteturn46file0L46-L48
...
This configuration tells Claude Desktop which Python interpreter to use, which HexStrike MCP script to launch, and which local HexStrike server endpoint to connect to. fileciteturn46file0L50-L59
...
The final section of the lab guide shows Claude Desktop displaying the HexStrike MCP server as connected and also shows a Claude conversation interacting with the HexStrike environment. This served as the final verification stage of the setup. fileciteturn46file0L64-L67
